(function() {
var fs, path, diacritic, marked, milk, uglify, cssshrink, renderers, ReticuleRenderer, TocRenderer, cache, readdirRecursive, fillTemplate, r, r1;
sign = function(x) {
...
/*@49:1*/
exports.renderToc = function(tocmd, dirname, currentpath) {
    return marked(tocmd, {
...
exports.buildReticule = function(dirname) {
    var execPath, template, bookinfo, renderer, macros, toc, tocmd, linear, alltags, items, keywords, i1, l1, item, nav;
    ...
    for (i1 = 0; i1 < l1.length; i1++) {
        ...
        fs.writeFileSync(path.join(dirname, item, 'index.html'), fillTemplate(template, {
            ...
            macros: macros,
            toc: exports.renderToc(tocmd, dirname, item),
            relative: relative,
    ...
    fs.writeFileSync(path.join(dirname, 'index.html'), fillTemplate(template, {
        ...
        toc: exports.renderToc(tocmd, dirname, './'),
        macros: macros,
        relative: './',
        main: [('<h1>' + bookinfo.title) + '</h1>', marked(bookinfo.author).replace('<p>', '<p class="meta">'), '<article>', '<h2>Table of Contents</h2>', exports.renderToc(tocmd, dirname, './').replace(/<ol/g, '<ul').replace(/<\/ol>/g, '</ul>'), '</article>', nav].join('\n')
    }));
    /*@284:5*/
    ...
    fs.writeFileSync(path.join(dirname, 'index/index.html'), fillTemplate(template, {
        title: bookinfo.title,
        section: 'Index',
        toc: exports.renderToc(tocmd, dirname, 'index/'),
        macros: macros,
        relative: '../',
